 ...
 * - //cdn... => https:
 */
function resolveScriptUrl(src, siteUrl) {
    const s = (src || "").trim();
    if (!s)
        return "";
...
function isSameBaseDomain(scriptUrl, baseDomain) {
    if (!baseDomain)
        return false;
    try {
...
function isTrackingScript(url) {
    ...
    if (u.includes("snap.licdn.com") || // LinkedIn Insight Tag
 ...
 */
function extractExtraTrackingUrlsFromHtml(html) {
    const out = [];
    if (!html)
        return out;
    const patterns = [
...
async function fetchRelevantExternalScripts(externalScripts, siteUrl) {
    ...
    const htmlCandidates = extractExtraTrackingUrlsFromHtml(globalThis.__PIXEL_AUDITOR_HTML__ || "");
    ...
    async function worker() {
        while (idx < candidates.length) {
            ...
            const isGtmContainer = lower.includes("googletagmanager.com/gtm.js") ||
                lower.includes("gtm.js?id=");
            const shouldExclude = isExcludedFromEvents && !isGtmContainer;
            try {
                const content = await (0, fetchPage_1.fetchExternalScript)(current);
                if (content) {
