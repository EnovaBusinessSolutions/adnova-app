...
exports.detectGTM = detectGTM;
exports.isValidGTMId = isValidGTMId;
exports.analyzeDataLayer = analyzeDataLayer;
 ...
 */
function detectGTM(pageContent, scripts) {
    const result = {
        detected: false,
        containers: [],
        errors: [],
    ...
    const scriptMatches = (0, extractScripts_1.extractAllMatches)(allContent, gtmScriptPattern);
    // Patrón 2: GTM en iframe (noscript fallback)
    const gtmIframePattern = /googletagmanager\.com\/ns\.html\?id=(GTM-[A-Z0-9]+)/gi;
    const iframeMatches = (0, extractScripts_1.extractAllMatches)(allContent, gtmIframePattern);
    // Patrón 3: Inicialización de dataLayer
    const dataLayerPattern = /dataLayer\s*=\s*(?:dataLayer\s*\|\|\s*)?\[\]/gi;
    const dataLayerInitMatches = (0, extractScripts_1.extractAllMatches)(allContent, dataLayerPattern);
    // Patrón 4: GTM ID en configuración manual (gtm.start)
    const gtmConfigPattern = /['"]gtm\.start['"]\s*:[\s\S]*?['"](GTM-[A-Z0-9]+)['"]/gi;
    const configMatches = (0, extractScripts_1.extractAllMatches)(allContent, gtmConfigPattern);
    // Patrón 5: GTM- ID en cualquier contexto de string
    const anyGtmIdPattern = /['"](GTM-[A-Z0-9]{5,10})['"]/gi;
    const anyGtmMatches = (0, extractScripts_1.extractAllMatches)(allContent, anyGtmIdPattern);
    ...
    const dynamicMatches = (0, extractScripts_1.extractAllMatches)(allContent, dynamicLoadPattern);
    // Patrón 11: GTM en window/global variable
    const windowPattern = /window\.(?:gtmId|GTM_ID|containerId)\s*=\s*['"](GTM-[A-Z0-9]+)['"]/gi;
    const windowMatches = (0, extractScripts_1.extractAllMatches)(allContent, windowPattern);
 ...
 */
function analyzeDataLayer(content) {
