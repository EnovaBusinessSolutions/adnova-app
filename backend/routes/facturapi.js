// backend/routes/facturapi.js
'use strict';
const express = require('express');
const router = express.Router();

const {
  emitirFactura,
  genCustomerPayload,
  enviarCfdiPorEmail,   // ⬅️ envío separado
} = require('../services/facturaService');

let User = null;       try { User = require('../models/User'); } catch {}
let TaxProfile = null; try { TaxProfile = require('../models/TaxProfile'); } catch {}

/**
 * POST /api/facturapi/emit
 * body: { userId, amount, description }
 * Timbrado manual rápido (total con IVA incluido) → luego envío por email.
 */
router.post('/emit', async (req, res) => {
  try {
    const { userId, amount, description } = req.body || {};
    if (!userId || amount == null) {
      return res.status(400).json({ ok: false, error: 'userId y amount requeridos' });
    }

    const totalWithTax = Number(amount);
    if (Number.isNaN(totalWithTax) || totalWithTax < 0.01) {
      return res.status(400).json({ ok: false, error: 'amount inválido (>= 0.01)' });
    }

    const user = await User.findById(userId).lean();
    if (!user) return res.status(404).json({ ok: false, error: 'Usuario no encontrado' });

    const taxProfile = TaxProfile ? await TaxProfile.findOne({ user: user._id }).lean() : null;
    const customer   = genCustomerPayload(taxProfile);
    const concept    = description || 'Emisión manual';
    const cfdiUse    = taxProfile?.cfdiUse;

    // 1) Timbrado (sin send_email/email/metadata)
    const cfdi = await emitirFactura({
      customer,
      description: concept,
      totalWithTax,
      cfdiUse,
    });

    // 2) Envío por correo desacoplado (no rompe si falla)
    let emailed = false;
    const destinatario = customer.email || user.email;
    if (destinatario) {
      try {
        await enviarCfdiPorEmail(cfdi.id, destinatario);
        emailed = true;
      } catch (e) {
        console.warn('[facturapi/emit] No se pudo enviar el CFDI por email (timbrado OK):',
          e?.response?.data || e.message);
      }
    }

    return res.json({ ok: true, cfdi, emailed, to: destinatario || null });
  } catch (e) {
    return res.status(500).json({ ok: false, error: e?.response?.data || e.message });
  }
});

module.exports = router;
