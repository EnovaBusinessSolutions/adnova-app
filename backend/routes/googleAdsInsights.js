...
const ENV_LOGIN_ID = (process.env.GOOGLE_ADS_LOGIN_CUSTOMER_ID || process.env.GOOGLE_LOGIN_CUSTOMER_ID || '')
  .replace(/-/g, '')
  .trim();
...
function headersFor(accessToken, managerIdFromDoc) {
  ...
  const login = docLogin || ENV_LOGIN_ID;
...
function selectedFromDocOrUser(gaDoc, req) {
  ...
  const fromUser = Array.isArray(req.user?.selectedGoogleAccounts)
    ? req.user.selectedGoogleAccounts.map(normId)
    : [];
  return [...new Set(fromUser.filter(Boolean))];
}
...
async function listAccessibleCustomers(accessToken, managerId) {
  ...
  try {
    const { data } = await axios.get(url, { headers: headersFor(accessToken, managerId), timeout: 20000, validateStatus: () => true });
    return (data?.resourceNames || []).map((rn) => rn.split('/')[1]).filter(Boolean);
  } catch (e) {
    console.warn('listAccessibleCustomers error:', axiosErrorInfo(e));
    return [];
  }
}
...
router.get('/', requireAuth, async (req, res) => {
  try {
    const gaDoc = await GoogleAccount.findOne({
      ...
      .select('+refreshToken +accessToken objective defaultCustomerId managerCustomerId loginCustomerId customers ad_accounts selectedCustomerIds')
      .lean();
    ...
    const [chunks, prevChunks, metaChunks] = await Promise.all([
      searchAny({ accessToken, customerId: requestedCustomer, query: qCurr, managerId }),
      searchAny({ accessToken, customerId: requestedCustomer, query: qPrev, managerId }),
      searchAny({ accessToken, customerId: requestedCustomer, query: qMeta, managerId }).catch(() => []),
    ]);
    ...
    const currency =
      meta0?.currencyCode ||
      customerList(gaDoc).find((c) => normId(c.id) === String(requestedCustomer))?.currencyCode ||
      'USD';
    const timeZone =
      meta0?.timeZone ||
      customerList(gaDoc).find((c) => normId(c.id) === String(requestedCustomer))?.timeZone ||
      'America/Mexico_City';
...
router.get('/accounts', requireAuth, async (req, res) => {
  try {
    const ga = await GoogleAccount
      .findOne({ $or: [{ user: req.user._id }, { userId: req.user._id }] })
      .select('+refreshToken +accessToken managerCustomerId loginCustomerId defaultCustomerId customers ad_accounts selectedCustomerIds')
      .lean();
    ...
    if (selected.length > 0 && defaultCustomerId && !selected.includes(normId(defaultCustomerId))) {
      ...
      if (defaultCustomerId) {
        await GoogleAccount.updateOne(
          { $or: [{ user: req.user._id }, { userId: req.user._id }] },
          { $set: { defaultCustomerId: normId(defaultCustomerId) } }
        );
      }
    }
