...
/* ---------- util: IDs únicos presentes en snapshot ---------- */
...
function topAccountsBySpend(snapshot, type, limit = 3) {
  ...
  const map = new Map(); // id -> spend
  for (const c of snapshot.byCampaign || []) {
    const id = norm(c.account_id || '');
    if (!id) continue;
    const k = c.kpis || {};
    const spend = Number(k[spendKey] || 0);
    map.set(id, (map.get(id) || 0) + spend);
  }
  ...
  // fallback si no hubo series/kpis
  if (!ranked.length) {
    return uniqueAccountIds(snapshot, type).slice(0, limit);
  }
  return ranked;
}
...
async function runAuditFor({ userId, type }) {
  try {
    ...
    const setupIssues = []; // para avisos de selección/auto-limit
    ...
    if (type === 'google') {
      ...
      if (allIds.length > 3 && !haveSelection) {
        ...
        setupIssues.push({
          ...
          area: 'setup',
          severity: 'media',
          title: 'Se auditaron las 3 cuentas con mayor gasto por defecto',
          evidence: `Cuentas totales: ${allIds.length}. Muestra usada: ${auto.join(', ')}.`,
          recommendation: 'En Conexiones → Google elige explícitamente las cuentas a auditar para un análisis 100% alineado.',
    ...
    if (type === 'meta') {
      ...
      if (allIds.length > 3 && !haveSelection) {
        ...
        setupIssues.push({
          ...
          severity: 'media',
          title: 'Se auditaron las 3 cuentas con mayor gasto por defecto',
          evidence: `Cuentas totales: ${allIds.length}. Muestra usada: ${auto.map(a => `act_${a}`).join(', ')}.`,
          recommendation: 'En Conexiones → Meta selecciona las cuentas específicas que quieras auditar.',
          estimatedImpact: 'medio'
        });
      } else {
    ...
    await Audit.create({
      ...
      summary: auditJson?.summary || (setupIssues.length ? setupIssues[0].title : 'Auditoría generada'),
      issues: mergedIssues,
    ...
    await Audit.create({
      ...
      issues: [{
        ...
        recommendation: 'Verifica la conexión y permisos. Si tienes varias cuentas, selecciona las que quieres auditar.',
