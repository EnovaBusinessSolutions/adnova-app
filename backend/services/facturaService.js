...
// Emisor (referencia); Público en general = 616
...
/** Construye el payload de cliente para Facturapi (o Público en general). */
...
/**
 * Timbra una factura con precio final IVA incluido.
 * - Facturapi exige: price en product.price y taxes dentro de product.
 * - No enviamos send_email/email en create (la API actual lo rechaza).
 * - Si se provee sendEmailTo, intentamos enviar el CFDI después del timbrado.
 */
async function emitirFactura({ customer, description, totalWithTax, cfdiUse, sendEmailTo, metadata }) {
  ...
  const invoice = await facturapi.invoices.create({
    ...
    // ❌ NO enviar place_of_issue / send_email / email en create
    metadata,
  });

  // Envío por correo (opcional) después del timbrado — no bloquea si falla.
  try {
