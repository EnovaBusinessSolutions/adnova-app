 ...
 */
function extractGA4Events(content: string): EventData[] {
  const events: EventData[] = [];
  ...
  const matches = extractAllMatches(content, gtagEventPattern);
  ...
  const minifiedMatches = extractAllMatches(content, minifiedPattern);
  ...
  const snakeMatches = extractAllMatches(content, snakeCasePattern);
 ...
 */
function extractGTMEvents(content: string): EventData[] {
  const events: EventData[] = [];
  
  // Patrón: dataLayer.push({ event: 'event_name', ... })
  const dataLayerPattern = /dataLayer\.push\s*\(\s*(\{[^}]*event[^}]*\})\s*\)/gi;
  const matches = extractAllMatches(content, dataLayerPattern);
 ...
 */
function extractMetaPixelEvents(content: string): EventData[] {
  const events: EventData[] = [];
  const foundEvents = new Set<string>();
  ...
  const trackMatches = extractAllMatches(content, fbqTrackPattern);
  ...
  const customMatches = extractAllMatches(content, fbqCustomPattern);
  ...
  const minifiedTrackMatches = extractAllMatches(content, minifiedTrackPattern);
  ...
  const minifiedCustomMatches = extractAllMatches(content, minifiedCustomPattern);
  ...
  const anyMinifiedMatches = extractAllMatches(content, anyMinifiedStandardPattern);
  ...
  const pascalMatches = extractAllMatches(content, pascalCasePattern);
  ...
  const customCaseMatches = extractAllMatches(content, customCasePattern);
  ...
  const initMatches = extractAllMatches(content, pixelInitPattern);
  
  // También buscar init en código minificado (ej: n("init","123456"))
  const minifiedInitPattern = /\w{1,3}\s*\(\s*['"]init['"]\s*,\s*['"](\d{10,20})['"]/gi;
  const minifiedInitMatches = extractAllMatches(content, minifiedInitPattern);
 ...
 */
export function validateEventParameters(events: EventData[]): Array<{
  event: EventData;
