...
export interface ShopifyEventConfig {
  type: string;
  actionLabels: string[];
  /** Plataforma del evento: 'GA4', 'MetaPixel', etc. */
  platform?: 'GA4' | 'MetaPixel' | 'TikTok';
}

export interface ShopifyPixelConfig {
  ga4Ids: string[];
  googleAdsIds: string[];
  googleTagIds: string[];      // GT- format (Google Tag)
  merchantCenterIds: string[]; // MC- format (Merchant Center)
  metaPixelIds: string[];
  tiktokPixelIds: string[];
  /** Eventos configurados en Shopify Web Pixels */
  configuredEvents: ShopifyEventConfig[];
  /** Si Meta Pixel está instalado via app oficial de Shopify */
  hasMetaPixelApp: boolean;
  otherPixels: Array<{
    type: string;
    id: string;
    apiClientId?: number | string;
  }>;
  isShopify: boolean;
}
 ...
 */
export function extractShopifyPixelsConfig(html: string): ShopifyPixelConfig {
  const result: ShopifyPixelConfig = {
    ga4Ids: [],
    googleAdsIds: [],
    googleTagIds: [],
    merchantCenterIds: [],
  ...
  if (webPixelsMatch) {
    try {
      ...
      pixelsConfig.forEach((pixel: any) => {
        if (!pixel.configuration) return;
        ...
        if (config.config) {
          try {
            ...
            if (innerConfig.google_tag_ids) {
              innerConfig.google_tag_ids.forEach((id: string) => {
                if (id.startsWith('G-')) {
            ...
            if (innerConfig.gtag_events) {
              innerConfig.gtag_events.forEach((event: any) => {
                // Guardar el evento configurado
                if (event.type) {
                  const eventConfig: ShopifyEventConfig = {
                    type: event.type,
                ...
                if (event.action_label) {
                  event.action_label.forEach((label: string) => {
 ...
 */
function extractGoogleIdsFromString(content: string, result: ShopifyPixelConfig): void {
 ...
 */
export function detectShopifyTrackingPatterns(html: string): {
  hasWebPixelsManager: boolean;
  hasMonorailTracking: boolean;
  hasTrekkieTracking: boolean;
  shopifyAppsDetected: string[];
} {
  return {
 ...
 */
function extractShopifyAppNames(html: string): string[] {
  const apps: string[] = [];
  
  // Detectar apps conocidas por su apiClientId
  const appMappings: Record<number, string> = {
