...
import { GoogleAdsResult, PageContent, ScriptInfo } from '../../type/AuditResult';
 ...
 */
export function detectGoogleAds(
  ...
  const scriptMatches = extractAllMatches(allContent, googleAdsScriptPattern);
  ...
  const conversionMatches = extractAllMatches(allContent, conversionPattern);
  ...
  const anyAwMatches = extractAllMatches(allContent, anyAwIdPattern);
  ...
  const linkerMatches = extractAllMatches(allContent, conversionLinkerPattern);
  ...
  const numericMatches = extractAllMatches(allContent, numericConversionPattern);
  
  // Patrón 11: goog_report_conversion
  const googReportPattern = /goog_report_conversion\s*\(\s*['"](AW-\d+)/gi;
  const googReportMatches = extractAllMatches(allContent, googReportPattern);
  ...
  conversionMatches.forEach(match => {
    if (match[1]) allConversions.push(match[1]);
  });
 ...
 */
export function extractGoogleAdsConfig(content: string): {
  ids: string[];
  conversions: string[];
