 ...
 *
 * Nota:
 * - Tag Assistant / GTM pueden escuchar page_view por History Change.
 * - Aquí disparamos un evento explícito para que marketing lo tenga 100% confiable.
 */
const START_SS_KEY = "adray:start:meta_ads:fired:v1";
...
function fireStartEvent(payload: Record<string, any>) {
  ...
  // Meta Pixel (fbq) — opcional (si quieren track también ahí)
  try {
    const w = window as any;
...
export default function MetaAds() {
   ...
   * - Solo cuando realmente estamos en la ruta /meta-ads/start
   * - Solo cuando ya hay ready (para evitar disparo en estado intermedio)
   */
  const isStartRoute = useMemo(() => {
    const p = (location.pathname || "").toLowerCase();
    return p.endsWith("/meta-ads/start") || p.endsWith("/meta-ads/start/");
  }, [location.pathname]);
  ...
  useEffect(() => {
    ...
    const payload = {
      ...
      level: effectiveLevel,
      day: effectiveDay || undefined,
      ts: Date.now(),
    };

    // Marca primero para evitar duplicados si algo re-renderiza rápido
